...
```bash
git clone --branch v0.19.9 --depth 1 https://github.com/XMZO/moemail-local.git
cd moemail-local
corepack enable
...
Keep the recipient header set to `delivered-to`. MoeMail normally trusts Mailu's Sieve-generated `Delivered-To`; when the [Mailu 2024.06 alias bug](https://github.com/Mailu/Mailu/issues/3587) writes the collector address instead, it recovers the original envelope recipient only after validating the exact two-local-LMTP-plus-Postfix trace at the start of the message, including the correlated LMTP transaction IDs. It never scans later sender-controlled `Received` fields or uses MIME `To`/`Cc`. The first upgraded run replays the old bounded UID range once, so messages skipped by the former parser are recovered through the same idempotent ingestion path. Real-time `IMAP IDLE` receiving is enabled by default: Mailu notifies MoeMail over the long-lived connection as soon as a message reaches the collector, disconnections reconnect with exponential backoff, and a configurable 15–86400 second full poll covers missed notifications and downtime. Servers without IDLE safely fall back to polling. SMTP sending requires both an active mailbox owned by the caller and its exact managed alias, so the catch-all never authorizes arbitrary From addresses. Password rotation is available only through the dedicated random-rotation buttons. By default, upstream messages are deleted 24 hours after MoeMail durably commits them; choose Keep, Archive, or a shorter delay if preferred. Deletion/move is queued only for committed messages or proven duplicates, requires safe UID-scoped IMAP capabilities, and leaves rejected or unknown-recipient mail upstream.
...
Select External mailbox IMAP for the domain in the WebUI, enter the IMAP host, port, TLS mode, username, password or app password, and mailbox, then test the connection and save. The connection test also reports whether the server advertises `IDLE`. New WebUI policies enable real-time mode by default: an IDLE-capable provider wakes MoeMail as soon as mail arrives, while a provider without IDLE automatically stays on complete polling. Existing saved IMAP policies remain polling-only until you enable the switch. By default, only mail arriving after the configuration is saved is imported; initial unread-message import is optional. The receiver runs inside the Web process and needs no Compose profile.

IDLE and polling both use read-only `EXAMINE` and PEEK operations, so they never mark messages read, move them, or delete them upstream. The configurable 15–86400 second full poll never turns off: it recovers missed notifications, provider outages, reconnect gaps, and messages arriving while MoeMail is stopped. Advanced settings expose the connection timeout, IDLE renewal interval, exponential reconnect range, and per-scan batch size. Reconnect can be disabled independently; polling still remains active. At most 32 read-only listeners and 4 concurrent polling jobs run at once, so additional domains degrade to polling instead of consuming unbounded sockets. Progress is persisted by `UIDVALIDITY + UID`, while raw RFC822 content provides application-level deduplication. Tail the Web container for status:
